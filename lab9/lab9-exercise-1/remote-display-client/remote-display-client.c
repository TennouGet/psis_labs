...
{	
    ...
    PayperviewReq display_req = PAYPERVIEW_REQ__INIT;
    ...
    display_req.client_name = "Joao Silva";
    ...
    PayperviewResp* server_response;

    msg_len = zmq_recvmsg(requester, &zmq_msg, 0);

    void * msg_data = zmq_msg_data (&zmq_msg);
    server_response = payperview_resp__unpack(NULL, msg_len, msg_data);
    ...
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, &server_response->secret, sizeof(server_response->secret));
