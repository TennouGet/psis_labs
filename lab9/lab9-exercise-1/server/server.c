...
{	
    ...
    while (1)
    {
        ...
        if(msg_type == 2){
            ...
            display_message = payperview_req__unpack(NULL, msg_len, msg_data);

            PayperviewResp display_response = PAYPERVIEW_RESP__INIT;

            printf("number: %s\n",display_message->client_credit_card);


            display_response.secret = 1;

            msg_len = payperview_resp__get_packed_size(&display_response);
            char * msg_buf = malloc(msg_len);
            payperview_resp__pack(&display_response, msg_buf);

            zmq_send(responder, msg_buf, sizeof(msg_len), 0);
